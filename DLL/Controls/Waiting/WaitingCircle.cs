...
{
    public class WaitingCircle : WaitingBase
    {
        public double DotOffSet { get => (double)GetValue(DotOffSetProperty); set => SetValue(DotOffSetProperty, value); }
        public static readonly DependencyProperty DotOffSetProperty = DependencyProperty.Register(nameof(DotOffSet), typeof(double), typeof(WaitingCircle), new FrameworkPropertyMetadata(20.0, FrameworkPropertyMetadataOptions.AffectsRender));

        public bool NeedHidden { get => (bool)GetValue(NeedHiddenProperty); set => SetValue(NeedHiddenProperty, ValueBoxes.BooleanBox(value)); }
        public static readonly DependencyProperty NeedHiddenProperty = DependencyProperty.Register(nameof(NeedHidden), typeof(bool), typeof(WaitingCircle), new FrameworkPropertyMetadata(true, FrameworkPropertyMetadataOptions.AffectsRender));

        static WaitingCircle()
        {
            DotSpeedProperty.OverrideMetadata(typeof(WaitingCircle), new FrameworkPropertyMetadata(6.0, FrameworkPropertyMetadataOptions.AffectsRender));

            DotDelayTimeProperty.OverrideMetadata(typeof(WaitingCircle), new FrameworkPropertyMetadata(220.0, FrameworkPropertyMetadataOptions.AffectsRender));
        }
        ...
        protected sealed override void UpdateDots()
        {
            if (DotCount > 0)
            {
                ...
                for (var i = 0; i < DotCount; i++)
                {
                    ...
                    var framesMove = new DoubleAnimationUsingKeyFrames { BeginTime = TimeSpan.FromMilliseconds(DotDelayTime * i) };
